...
const filter = () => {
  ...
  const event = (btn, mark) => {
    btn.addEventListener('click', () => {
      typeFilter(mark);
    });
  };
  event(btnAll, markAll);
  event(btnLovers, markLovers);
  event(btnChef, markChef);
  event(btnGirl, markGirl);
  event(btnGuy, markGuy);
  event(btnGrandmother);
  event(btnGranddad);
